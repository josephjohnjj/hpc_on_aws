...
In this setup, all nodes are running `Rocky Linux 9.6 (Blue Onyx`). The cluster machines 
will be referred to as `node` or `nodes`, while the laptop or desktop will be referred 
to as the `local system`.

When the VMs are created, an SSH key, `terraform-user`, is already added, making it easy to 
log in from the local system using `~/.ssh/terraform-user`.

Each node in the system has three hard disks: `/dev/nvme0n1`, `/dev/nvme1n1` and `/dev/nvme3n1`, 
with `/dev/nvme0n1` containing the root filesystem.


Initial packages to install on all nodes
------------------------------------------

OpenPBS does not work with SELinux enabled. Therefore, the first step is to disable SELinux on all nodes:

.. code-block:: bash

    sudo setenforce 0

To ensure the change persists even after a reboot, edit the file `/etc/selinux/config` and set:  

.. code-block:: bash

    SELINUX=disabled

Next, enable the CRB repository and the PLUS repository on all nodes:
...
Refresh the package metadata and clear any old cached data:
...
Update the GNU C Library (glibc) first to avoid dependency issues before updating the rest 
of the system:
...
Install EPEL (Extra Packages for Enterprise Linux)to access additional open-source packages.
...
Ensure the installed packages match the currently enabled repositories versions:
...
Installs compiler tools (gcc, make, etc.) as a group:
...
Install commonly needed utilities and compilers:

.. code-block:: bash

    sudo dnf install -y wget vim telnet net-tools python3 python3-devel perl 
    sudo dnf install -y autoconf automake gcc gcc-c++ make cmake chkconfig nmap-ncat
...
The main pain points when it came to intalling the packges where:

1. Discrepancy in the glibc package version: There is a version mismatch or inconsistency in the installed GNU C Library (glibc) package 
across the system or between nodes. The glibc library is a critical component that provides 
the core system libraries for applications and system utilities to function correctly. 
A discrepancy in its version can lead to compatibility issues, unexpected behavior 
in binaries compiled against different library versions, or errors during package i
nstallation and system updates. 

2. Mismatch between the kernel version and the kernel headers version:The installed kernel version does not match the kernel-headers package version. Kernel 
headers are used during software compilation (especially for drivers and kernel modules) 
and must correspond exactly to the running kernel. When the versions differ, it can cause 
build failures for kernel-dependent packages or modules, and may lead to runtime errors if 
compiled against the wrong headers. 
...
This creates a new keypair (id_rsa private key and id_rsa.pub public key). Once the keys are 
created, copy the key back to the local system. 
...
.. code-block:: bash
    ...
    scp -i ~/.ssh/terraform-user rocky@<node2 ip>:~/.ssh/id_rsa node2.pub
    scp -i ~/.ssh/terraform-user rocky@<node3 ip>:~/.ssh/id_rsa node3.pub
    scp -i ~/.ssh/terraform-user rocky@<node4 ip>:~/.ssh/id_rsa node4.pub
    scp -i ~/.ssh/terraform-user rocky@<node5 ip>:~/.ssh/id_rsa node5.pub
    scp -i ~/.ssh/terraform-user rocky@<node6 ip>:~/.ssh/id_rsa node6.pub
    scp -i ~/.ssh/terraform-user rocky@<node7 ip>:~/.ssh/id_rsa node7.pub


Set the hostname 
~~~~~~~~~~~~~~~~~~~~~
Set the hostname on each node. For example, on the first node, run:

.. code-block:: bash

    sudo hostnamectl set-hostname node1

Update `/etc/hostname` with the new hostname: The **hostname** is the unique name assigned 
to a system on a network. It is used to identify the machine in communications with other 
nodes, for logging, and for network management.
...
Next, verify that the **localhost** and **IPv6** entries are correctly set in `/etc/hosts`:
The `/etc/hosts` file maps hostnames to IP addresses. Ensuring correct entries for `localhost` 
and IPv6 addresses allows the system to properly resolve its own name and handle network 
communications internally, which is essential for many services and applications.
...
Finally, add the details of all nodes along with their local IP addresses to `/etc/hosts`. 
To find the local (private) IP address on RHEL, you can use one of the following command:

.. code-block:: bash

    ip addr show

These commands display the IP addresses assigned to the network interfaces on the node, 
which you can then use in `/etc/hosts` for proper hostname-to-IP mapping. Example:
...
At the end, the `/etc/hosts` file should look like this:
...
On the local system, combine all the SSH public keys from the cluster nodes into a single 
authorized_keys file. This allows each node to recognize the keys of all other nodes for
passwordless SSH:
...
ensures that all nodes have the same set of authorized keys:

.. code-block:: bash

    scp -i ~/.ssh/terraform-user authorized_keys rocky@<node1 ip>:~/.ssh/authorized_keys 

Finally, on each node, set the correct permissions for the authorized_keys file to ensure SSH 
works securely:
...
Remove the old SSH host key for a specific host (in this case, `<other-node-ip>`) from the 
local machine's `~/.ssh/known_hosts` file. This helps prevent SSH warnings or connection 
issues when a host's key has changed.
...
Next, retrieve the SSH host key from the remote machine (`<other-node-ip>`) and add it to 
...
Next, edit the `~/.ssh/config` file to include the following settings:
...
Then, edit `/etc/ssh/sshd_config` on each node and ensure the following:

* `PasswordAuthentication yes` - allows password authentication if needed.

* `ChallengeResponseAuthentication no` - disables challenge-response authentication.

* `UsePAM yes` - enables Pluggable Authentication Modules (PAM) for additional authentication 
support.
...
Deletes the SSH configuration file is important because cloud images often include default 
SSH settings that can disable password login or override your manual SSH configuration.

In `/etc/cloud/cloud.cfg` (create the file if it doesn't exist), enable password authentication 
to ensure cloud-init does not overwrite your SSH settings on reboot or redeployment:
...
* Setting `ssh_pwauth: 1` allows SSH password login.
* This prevents cloud-init from resetting or disabling password authentication during instance reboots or redeployments, ensuring your manual SSH configuration remains intact.
...
that the SSH key setup and `authorized_keys` configuration are correct.
...
Then create a systemd service file for Prometheus:
...
the download and install Node Exporter:
...
Then create a systemd service file for Node Exporter:
...
First, import the Grafana GPG key:
...
Then create the Grafana repository file:
...
Finally, access the Grafana web interface by navigating to the following URL in your web
browser:
